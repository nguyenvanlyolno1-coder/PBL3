...
public static void lockNetwork(String allowedUrlsCsv) {
    try {
        ...
        // ✅ QUAN TRỌNG: Cho phép kết nối TCP đang mở (WebSocket)
            ...
            for (String domain : allowedUrlsCsv.split(",")) {
                ...
                try {
                    ...
                    for (java.net.InetAddress addr : addresses) {
                        ...
                        rules.append("iptables -A OUTPUT -d ").append(ip).append(" -j ACCEPT && ");
                        rules.append("iptables -A INPUT -s ").append(ip).append(" -j ACCEPT && ");
                        System.out.println("  ✅ Cho phép: " + domain + " → " + ip);
                    }
                } catch (Exception e) {
                    System.out.println("  ⚠️ Không resolve: " + domain);
                }
            }
        }
        ...
        try {
            ...
            for (java.net.InetAddress addr : ngrokAddrs) {
                ...
                rules.append("iptables -A OUTPUT -d ").append(ip).append(" -j ACCEPT && ");
                rules.append("iptables -A INPUT -s ").append(ip).append(" -j ACCEPT && ");
                System.out.println("  ✅ Cho phép ngrok: " + ip);
            }
        } catch (Exception e) {
            System.out.println("  ⚠️ Không resolve ngrok: " + ngrokHost);
        }
        ...
        if (exitCode == 0) {
            System.out.println("🔒 Khóa mạng thành công!");
        } else {
            System.out.println("❌ Lỗi: " + stderr);
        }
